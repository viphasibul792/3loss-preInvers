...
let lossStreak = 0; // লস কাউন্টার ভেরিয়েবল যুক্ত করা হয়েছে
...
async function checkAndPredict() {
    ...
    try {
        ...
        if (currentPeriod !== lastAnalyzedPeriod && seconds >= 10 && seconds <= 15) {
            ...
            // --- Loss Streak Logic Implementation ---
            if (lossStreak === 4) {
                // প্রেডিকশন উল্টে দেওয়া
                prediction = (prediction === "BIG") ? "SMALL" : "BIG";
                lossStreak = 0; // কাউন্টার রিসেট
            }
            // ---------------------------------------
